...
const columns: ColumnsType<DataType> = [
  {
    ...
    dataIndex: 'address',
    key: 'address',
    render: (address) => <p >{address.substring(0, 6).concat('...').concat(address.substring(address.length - 5)) }</p>
  },
  {
    title: 'Amount',
    dataIndex: 'amount',
  ...
  },
  {
    ...
    render: (transaction: string) => (
            <Space size="middle">
                <a href={`https://etherscan.io/tx/${transaction}`} >{transaction.substring(0, 6).concat('...').concat(transaction.substring(transaction.length - 5)) }</a>
            </Space>
    )
